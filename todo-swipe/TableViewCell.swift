...
class TableViewCell : UITableViewCell {
  ...
  var delegate: TableViewCellDegelate?
  ...
  func handlePan(recognizer: UIPanGestureRecognizer) {
    if recognizer.state == .Began {
      // record current center location
    ...
    if recognizer.state == .Changed {
      ...
      // has the user dragged the item far enough to initate a delete/complete?
      // "far enough" is half the width of the screen
    ...
    if recognizer.state == .Ended {
      ...
      if deleteOnDragRelease {
        if delegate != nil && toDoItem != nil {
          // notify the delegate tha this item should be deleted
        ...
        // Still snap back to the original location
        UIView.animateWithDuration(0.2, animations: {self.frame = originalFrame})
      } else {
        // item is not being deleted, so snap back to original location
        UIView.animateWithDuration(0.2, animations: {self.frame = originalFrame})
      }
    }
  }
